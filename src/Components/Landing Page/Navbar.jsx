            ...
            <div className='desktop px-20 py-2  fixed top-0 left-0 right-0 z-10 ' id='desktop'>
                <div className='flex justify-between items-center'>
                    ...
                    <ul className='flex  justify-between gap-x-8 '>
                        <a href="" className=' mt-1 list'><Link to='/' onClick={() => setLoading(true)}><li className=' text-lg font-bold '>HOME</li></Link></a>
                        <a href="" className=' mt-1 list'><li className=' text-lg font-bold '>ABOUT</li></a>
                        <a href="" className=' mt-1 list'><Link to="/products" onClick={() => setLoading(true)}><li className=' text-lg font-bold '>PRODUCTS</li></Link></a>
                        <a href="" className=' mt-1 list'><Link to="/myorder" onClick={() => setLoading(true)}><li className=' text-lg font-bold '>MY ORDERS</li></Link></a>
                    </ul>
                    {verifyuser ? (
            ...
            {showicons && (
                <div className={`mobilemenu ${showicons ? 'show-menu' : ''}`}>
                    ...
                    <div className='w-full h-screen  px-4 text-center mt-5'>
                        <ul className=''>
                            <Link to="/" onClick={() => setLoading(true)}><li className='py-4 text-lg cursor-pointer font-medium moblist' onClick={show}>HOME</li></Link>
                            <Link to="/" onClick={() => setLoading(true)}><li className='py-4 text-lg cursor-pointer font-medium moblist' onClick={show}>ABOUT</li></Link>
                            <Link to="/products" onClick={() => setLoading(true)}><li className='py-4 text-lg cursor-pointer font-medium moblist' onClick={show}>PRODUCTS</li></Link>
                            <Link to="/myorder" onClick={() => setLoading(true)}><li className='py-4 text-lg cursor-pointer font-medium moblist' onClick={show}>MY ORDERS</li></Link>
